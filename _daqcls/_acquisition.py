		...
		if savefile:
			...
			if group:
				try:
					...
					self.group	=   self.savefile[group]
			else:
...
class ContVoltDAQ(AIVoltageDAQ):
	...
	def __init__(
		...
		AIVoltageDAQ.__init__(self,sr,tr,device,savefile=savefile,group=group)
	...
	def __every_n_cb(self, task_handle, every_n_samples_event_type,
		...
		samples =   self.read(number_of_samples_per_channel=self.tr)
